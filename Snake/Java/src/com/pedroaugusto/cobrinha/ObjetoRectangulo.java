...
public abstract class ObjetoRectangulo {
    ...
    public void drawBorder(Graphics2D canvas, Color c, float tam)
    {
        canvas.setColor(c);
        Shape s;
        if(!nome.equals("Moeda"))
            s = new Rectangle2D.Float((getX()-tam), (getY()-tam),
                (getWidth()+tam), (getHeight()+tam));
        else
            s = new Ellipse2D.Double(getX()-tam, getY()-tam, getWidth()+tam, getWidth()+tam);
        canvas.fill(s);
    }
